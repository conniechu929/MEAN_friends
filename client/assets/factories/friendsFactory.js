...
app.factory('friendsFactory', ['$http', function($http) {
 ...
 //But only if you only want to run the callback from the controller.
    ...
    this.delete = function(friend, id, callback){// what parameters do we need?
      $http.delete('/friends/' + id).then(function(returned_data){
        console.log("FACTORY DELETED", returned_data.data);
        if (friend.id) {
          friends.splice(id, 1);
        }
        if(typeof(callback) == 'function') {
    ...
    this.show = function(id, callback){// what parameters do we need?
        $http.get('/friends/' + id).then(function(returned_data) {
          console.log("FACTORY FOUND",returned_data.data);
